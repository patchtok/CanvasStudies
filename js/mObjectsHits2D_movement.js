...
var colour1 = ['#ff0000' , '#0000ff', '#cccccc', '#882233'];
var objectSet = [];
var numberObjects = 0;
var minX = 0.1*width;
var maxX = 0.9*width;
...
Planet.prototype.draw = function(){	
	this.circleMake(this.radius,true, false);
    writeToBox(this.x, this.y, this.planetName);
...
// instantiation
var masses = [15,40, 25,5,60];
var speeds = [1, 2, 1,6,0];
var xP = [0.2, 0.8, 0.5, 0.2, 0.5];
var yP = [0.2, 0.2, 0.8, 0.8, 0.5];
var tAngles = [0,180, 270, -45,0];



numberObjects = masses.length;
for (var i = 0; i < numberObjects ; i++ ){
var obj="m" + i.toString();
var startX = xP[i]*width;
var startY = yP[i]*height;
var rad = masses[i];    
var tCol = colour1[i%colour1.length];
var setSpeed=Math.abs(speeds[i]);
var setAngle = tAngles[i]*Math.PI/180;
...
for( var i = 0; i < objectSet.length - 1 ; i++){
    var ck = false;
        for( var k = i+1 ; k < objectSet.length ; k++){
            var x1 = objectSet[i].getX();
            var y1 = objectSet[i].getY();
            var x2 = objectSet[k].getX();
            var y2 = objectSet[k].getY();
            var r1 = objectSet[i].getRadius();
            var r2 = objectSet[k].getRadius();
                ck =distanceCheck(x1,y1,x2,y2,r1,r2);
            if(ck){
                var m_2Angle = angleCalculate(x2, y2, x1 , y1);
                document.getElementById("angle2").innerHTML = "m_2 angle: "+Math.round(m_2Angle*180/Math.PI);
                var contactAngle = (m_2Angle+Math.PI/2)%(2*Math.PI);
                document.getElementById("contactAngle").innerHTML = "Contact angle: "+ Math.round((contactAngle)*180/Math.PI);
                var v_1 = objectSet[i].getSpeed();
                var v_2 = objectSet[k].getSpeed();
                var a_1 = objectSet[i].getAngle();
                var a_2 = objectSet[k].getAngle();
                var u_1 = v_1*Math.sin(a_1 - contactAngle);
                var u_2 = v_2*Math.sin(a_2 - contactAngle);
                ...
                ang_1 = ang_1 + contactAngle;
                ang_2 = ang_2 + contactAngle;
                objectSet[i].setSpeed(v_1new);
                objectSet[i].setAngle(ang_1);
                objectSet[k].setSpeed(v_2new);
                objectSet[k].setAngle(ang_2);
